...
const Row: React.FC<{
  ...
  <div className="mo-pick-row" role="group" aria-label={label}>
    ...
    <div className="mo-pick-opts">
      {options.map((o) => (
        <button
          ...
          aria-label={`${label}: ${(LABELS[kind] as Record<string, string>)[o]}`}
  ...
  return (
    <div className="mo-picker">
      <style>{`
        ...
        .mo-pick-opts{display:flex;flex-wrap:wrap;gap:8px;flex:1;min-width:0}
        ...
        .mo-pick-colors{display:flex;flex-wrap:wrap;gap:8px}
        ...
        .mo-picker button:focus-visible{outline:3px solid #4D7CFE;outline-offset:3px}
        .mo-picker button{font:inherit}
        @media(prefers-reduced-motion:reduce){.mo-picker *{transition:none!important}}
      `}</style>
      ...
      <Row label="Accesorio"  kind="hat"  options={HAT_VARIANTS}  active={value.hat}  color={value.color} onPick={(v) => set({ hat: v as HatVariant })} />

      <div className="mo-pick-row" role="group" aria-label="Color">
